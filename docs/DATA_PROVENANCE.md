...
6. **Semantic audit (Phase 5)** — Mapping approve/reject/edit/revert actions append to `semantic_audit_log` with previous/new state snapshots. Raw source identity in registry is never modified; semantic layer is separate metadata on `raw_points.metadata`.
